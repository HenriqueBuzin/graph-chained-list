...
int get_dist(vertice_t * v){
	return v->dist;
}
int get_visitado(vertice_t * v){
	return v->visitado;
}
vertice_t * get_pai(vertice_t * v){
	return v->pai;
}
void set_dist(vertice_t * v, int dist){
	v->dist = dist;
}
void set_visitado(vertice_t * v, int visitado){
	v->visitado = visitado;
}
...
void vertice_imprimi(vertice_t * v){

	printf("ID: %d, Distância: %d, Visitado: %d, Pai: %d\n", v->id, v->dist, v->visitado,
	v->pai == NULL ? -1 : v->pai->id
	);
